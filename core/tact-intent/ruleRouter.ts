...
// 挨拶や定型的なお礼は、外部情報・会話推論・LLM応答を必要としない。
// 判定はIntent Routerに集約し、Conversation層はこの結果だけを利用する。
const SIMPLE_CHAT_RESPONSES: ReadonlyMap<string, readonly string[]> = new Map([
  ["こんにちは", [
    "こんにちは！今日は何を進めますか？",
  ...
  ["はじめまして", [
    "はじめまして！今日は何を進めますか？",
    "はじめまして！何でも気軽に声をかけてください。",
  ]],
  ["お疲れ様", [
    "お疲れさまです。何かお手伝いできることはありますか？",
    "お疲れさまです。ひと息つきながら、次に進めることを考えましょう。",
  ]],
  ["おつかれさま", [
    "お疲れさまです。何かお手伝いできることはありますか？",
    "お疲れさまです。ひと息つきながら、次に進めることを考えましょう。",
...
function normalizeSimpleChatInput(input: string): string {
  return input
    .normalize("NFKC")
    .trim()
...
export function getSimpleChatResponse(input: string): string | undefined {
  const responses = SIMPLE_CHAT_RESPONSES.get(normalizeSimpleChatInput(input));
